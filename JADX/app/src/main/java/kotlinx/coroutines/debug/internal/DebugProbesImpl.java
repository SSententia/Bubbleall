...
public final class DebugProbesImpl {
    ...
    private final Function1<Boolean, Unit> getDynamicAttach() {
        ...
        try {
            ...
            if (objNewInstance != null) {
                objM751constructorimpl = Result.m751constructorimpl((Function1) TypeIntrinsics.beforeCheckcastToFunctionOfArity(objNewInstance, 1));
                if (Result.m757isFailureimpl(objM751constructorimpl)) {
                    objM751constructorimpl = null;
                }
                return (Function1) objM751constructorimpl;
            }
    ...
    private final <R> List<R> dumpCoroutinesInfoImpl(final Function2<? super CoroutineOwner<?>, ? super CoroutineContext, ? extends R> create) {
        ...
        try {
            ...
            List<R> list = SequencesKt.toList(SequencesKt.mapNotNull(SequencesKt.sortedWith(CollectionsKt.asSequence(debugProbesImpl.getCapturedCoroutines()), new C1851x8808a3e8()), new Function1<CoroutineOwner<?>, R>() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl$dumpCoroutinesInfoImpl$1$3
    ...
    private final List<StackTraceElement> enhanceStackTraceWithThreadDumpImpl(String state, Thread thread, List<StackTraceElement> coroutineTrace) {
        ...
        try {
            Result.Companion companion = Result.INSTANCE;
            DebugProbesImpl debugProbesImpl = this;
            objM751constructorimpl = Result.m751constructorimpl(thread.getStackTrace());
        } catch (Throwable th) {
            Result.Companion companion2 = Result.INSTANCE;
            objM751constructorimpl = Result.m751constructorimpl(ResultKt.createFailure(th));
        }
        ...
        StackTraceElement[] stackTraceElementArr = (StackTraceElement[]) objM751constructorimpl;
    ...
    private final Pair<Integer, Integer> findContinuationStartIndex(int indexOfResumeWith, StackTraceElement[] actualTrace, List<StackTraceElement> coroutineTrace) {
        ...
        while (i < 3) {
            ...
            if (iFindIndexOfFrame != -1) {
                return TuplesKt.m502to(Integer.valueOf(iFindIndexOfFrame), Integer.valueOf(i));
            }
            i = i2;
        }
        return TuplesKt.m502to(-1, 0);
    }
