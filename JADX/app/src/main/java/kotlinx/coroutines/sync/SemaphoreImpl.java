...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM2310constructorimpl;
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM2310constructorimpl = SegmentOrClosed.m2310constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM2310constructorimpl = SegmentOrClosed.m2310constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m2315isClosedimpl(objM2310constructorimpl)) {
                Segment segmentM2313getSegmentimpl = SegmentOrClosed.m2313getSegmentimpl(objM2310constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= segmentM2313getSegmentimpl.getId()) {
                        break loop0;
                    }
                    if (!segmentM2313getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    }
                    if (C0176xc40028dd.m82m(tail$FU, this, segment, segmentM2313getSegmentimpl)) {
                    ...
                    if (segmentM2313getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM2313getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m2313getSegmentimpl(objM2310constructorimpl);
        int i = (int) (andIncrement % ((long) SemaphoreKt.SEGMENT_SIZE));
        if (!ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m641m(semaphoreSegment3.acquirers, i, null, cont)) {
            if (ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m641m(semaphoreSegment3.acquirers, i, SemaphoreKt.PERMIT, SemaphoreKt.TAKEN)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        loop0: while (true) {
            ...
            if (!SegmentOrClosed.m2315isClosedimpl(objM2310constructorimpl)) {
                ...
                while (true) {
                    ...
                    if (C0176xc40028dd.m82m(head$FU, this, segment, segmentM2313getSegmentimpl)) {
                        if (!segment.decPointers$kotlinx_coroutines_core()) {
                            break loop0;
                        }
                        segment.remove();
        ...
        if (andSet == null) {
            ...
            return !ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m641m(semaphoreSegment3.acquirers, i2, SemaphoreKt.PERMIT, SemaphoreKt.BROKEN);
        }
