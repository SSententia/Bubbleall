...
class EngineJob<R> implements DecodeJob.Callback<R>, FactoryPools.Poolable {
    ...
    void notifyCallbacksOfResult() {
        synchronized (this) {
            ...
            for (ResourceCallbackAndExecutor resourceCallbackAndExecutor : resourceCallbacksAndExecutorsCopy) {
                resourceCallbackAndExecutor.executor.execute(new CallResourceReady(resourceCallbackAndExecutor.f163cb));
            }
    ...
    void notifyCallbacksOfException() {
        synchronized (this) {
            ...
            for (ResourceCallbackAndExecutor resourceCallbackAndExecutor : resourceCallbacksAndExecutorsCopy) {
                resourceCallbackAndExecutor.executor.execute(new CallLoadFailed(resourceCallbackAndExecutor.f163cb));
            }
            decrementPendingCallbacks();
        }
    }
    ...
    private class CallLoadFailed implements Runnable {
        ...
        private final ResourceCallback f161cb;

        CallLoadFailed(ResourceCallback resourceCallback) {
            this.f161cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f161cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f161cb)) {
                        EngineJob.this.callCallbackOnLoadFailed(this.f161cb);
                    }
    ...
    private class CallResourceReady implements Runnable {
        ...
        /* JADX INFO: renamed from: cb */
        private final ResourceCallback f162cb;

        CallResourceReady(ResourceCallback resourceCallback) {
            this.f162cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f162cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f162cb)) {
                        EngineJob.this.engineResource.acquire();
                        EngineJob.this.callCallbackOnResourceReady(this.f162cb);
                        EngineJob.this.removeCallback(this.f162cb);
                    }
                    EngineJob.this.decrementPendingCallbacks();
                }
            }
    ...
    static final class ResourceCallbackAndExecutor {
        ...
        /* JADX INFO: renamed from: cb */
        final ResourceCallback f163cb;
        final Executor executor;

        ResourceCallbackAndExecutor(ResourceCallback resourceCallback, Executor executor) {
            this.f163cb = resourceCallback;
            this.executor = executor;
        }

        public boolean equals(Object obj) {
            if (obj instanceof ResourceCallbackAndExecutor) {
                return this.f163cb.equals(((ResourceCallbackAndExecutor) obj).f163cb);
            }
            return false;
        }

        public int hashCode() {
            return this.f163cb.hashCode();
        }
    }
