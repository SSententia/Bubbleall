...
public final class FramedStream {
    ...
    public boolean isLocallyInitiated() {
        return this.connection.client == ((this.f236id & 1) == 1);
    }
    ...
    public void reply(List<Header> list, boolean z) throws IOException {
        ...
        this.connection.writeSynReply(this.f236id, z2, list);
        if (z2) {
    ...
    public void close(ErrorCode errorCode) throws IOException {
        if (closeInternal(errorCode)) {
            this.connection.writeSynReset(this.f236id, errorCode);
        }
    }

    public void closeLater(ErrorCode errorCode) {
        if (closeInternal(errorCode)) {
            this.connection.writeSynResetLater(this.f236id, errorCode);
        }
    }
    ...
    private boolean closeInternal(ErrorCode errorCode) {
        synchronized (this) {
            ...
            notifyAll();
            this.connection.removeStream(this.f236id);
            return true;
        }
    }
        ...
        public long read(Buffer buffer, long j) throws IOException {
            ...
            synchronized (FramedStream.this) {
                ...
                if (FramedStream.this.unacknowledgedBytesRead >= FramedStream.this.connection.okHttpSettings.getInitialWindowSize(65536) / 2) {
                    FramedStream.this.connection.writeWindowUpdateLater(FramedStream.this.f236id, FramedStream.this.unacknowledgedBytesRead);
    ...
    final class FramedDataSink implements Sink {
        ...
        private void emitDataFrame(boolean z) throws IOException {
            ...
            FramedStream.this.writeTimeout.enter();
            try {
                FramedStream.this.connection.writeData(FramedStream.this.f236id, z && jMin == this.sendBuffer.size(), this.sendBuffer, jMin);
            } finally {
                FramedStream.this.writeTimeout.exitAndThrowIfTimedOut();
            }
        }
            ...
            synchronized (FramedStream.this) {
                ...
                if (!FramedStream.this.sink.finished) {
                    if (this.sendBuffer.size() > 0) {
                        ...
                        FramedStream.this.connection.writeData(FramedStream.this.f236id, true, null, 0L);
                    }
                }
                synchronized (FramedStream.this) {
                    this.closed = true;
