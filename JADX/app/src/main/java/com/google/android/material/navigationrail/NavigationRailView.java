...
public class NavigationRailView extends NavigationBarView {
    ...
    public NavigationRailView(Context context, AttributeSet attributeSet) {
        this(context, attributeSet, C1218R.attr.navigationRailStyle);
    }

    public NavigationRailView(Context context, AttributeSet attributeSet, int i) {
        this(context, attributeSet, i, C1218R.style.Widget_MaterialComponents_NavigationRailView);
    }
    ...
    public NavigationRailView(Context context, AttributeSet attributeSet, int i, int i2) {
        ...
        this.topMargin = getResources().getDimensionPixelSize(C1218R.dimen.mtrl_navigation_rail_margin);
        TintTypedArray tintTypedArrayObtainTintedStyledAttributes = ThemeEnforcement.obtainTintedStyledAttributes(getContext(), attributeSet, C1218R.styleable.NavigationRailView, i, i2, new int[0]);
        int resourceId = tintTypedArrayObtainTintedStyledAttributes.getResourceId(C1218R.styleable.NavigationRailView_headerLayout, 0);
        if (resourceId != 0) {
            addHeaderView(resourceId);
        }
        setMenuGravity(tintTypedArrayObtainTintedStyledAttributes.getInt(C1218R.styleable.NavigationRailView_menuGravity, 49));
        if (tintTypedArrayObtainTintedStyledAttributes.hasValue(C1218R.styleable.NavigationRailView_itemMinHeight)) {
            setItemMinimumHeight(tintTypedArrayObtainTintedStyledAttributes.getDimensionPixelSize(C1218R.styleable.NavigationRailView_itemMinHeight, -1));
        }
        if (tintTypedArrayObtainTintedStyledAttributes.hasValue(C1218R.styleable.NavigationRailView_paddingTopSystemWindowInsets)) {
            this.paddingTopSystemWindowInsets = Boolean.valueOf(tintTypedArrayObtainTintedStyledAttributes.getBoolean(C1218R.styleable.NavigationRailView_paddingTopSystemWindowInsets, false));
        }
        if (tintTypedArrayObtainTintedStyledAttributes.hasValue(C1218R.styleable.NavigationRailView_paddingBottomSystemWindowInsets)) {
            this.paddingBottomSystemWindowInsets = Boolean.valueOf(tintTypedArrayObtainTintedStyledAttributes.getBoolean(C1218R.styleable.NavigationRailView_paddingBottomSystemWindowInsets, false));
        }
