...
public class WorkManagerUtil extends zzbt {
    ...
    public final boolean zze(IObjectWrapper iObjectWrapper, String str, String str2) throws Throwable {
        Context context = (Context) ObjectWrapper.unwrap(iObjectWrapper);
        zzb(context);
        Constraints constraintsBuild = new Constraints.Builder().setRequiredNetworkType(NetworkType.CONNECTED).build();
        try {
            WorkManager.getInstance(context).enqueue(new OneTimeWorkRequest.Builder(OfflineNotificationPoster.class).setConstraints(constraintsBuild).setInputData(new Data.Builder().putString(MediaConstants.MEDIA_URI_QUERY_URI, str).putString("gws_query_id", str2).build()).addTag("offline_notification_work").build());
            return true;
