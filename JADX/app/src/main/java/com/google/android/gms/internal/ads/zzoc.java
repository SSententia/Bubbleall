package com.google.android.gms.internal.ads;

import java.io.IOException;

/* JADX INFO: compiled from: com.google.android.gms:play-services-ads@@20.5.0 */
/* JADX INFO: loaded from: classes2.dex */
public final class zzoc {
    public static boolean zza(zzamf zzamfVar, zzoh zzohVar, int i, zzob zzobVar) {
        int iZzc;
        int iZzg = zzamfVar.zzg();
        long jZzt = zzamfVar.zzt();
        long j = jZzt >>> 16;
        if (j != i) {
            return false;
        }
        boolean z = (j & 1) == 1;
        int i2 = (int) ((jZzt >> 12) & 15);
        int i3 = (int) ((jZzt >> 8) & 15);
        int i4 = (int) (15 & (jZzt >> 4));
        int i5 = (int) ((jZzt >> 1) & 7);
        long j2 = jZzt & 1;
        if (i4 <= 7) {
            if (i4 != zzohVar.zzg - 1) {
                return false;
            }
        } else if (i4 > 10 || zzohVar.zzg != 2) {
            return false;
        }
        if (!(i5 == 0 || i5 == zzohVar.zzi) || j2 == 1 || !zzd(zzamfVar, zzohVar, z, zzobVar) || (iZzc = zzc(zzamfVar, i2)) == -1 || iZzc > zzohVar.zzb) {
            return false;
        }
        int i6 = zzohVar.zze;
        if (i3 != 0) {
            if (i3 <= 11) {
                if (i3 != zzohVar.zzf) {
                    return false;
                }
            } else if (i3 == 12) {
                if (zzamfVar.zzn() * 1000 != i6) {
                    return false;
                }
            } else {
                if (i3 > 14) {
                    return false;
                }
                int iZzo = zzamfVar.zzo();
                if (i3 == 14) {
                    iZzo *= 10;
                }
                if (iZzo != i6) {
                    return false;
                }
            }
        }
        return zzamfVar.zzn() == zzamq.zzW(zzamfVar.zzi(), iZzg, zzamfVar.zzg() + (-1), 0);
    }

    public static long zzb(zznv zznvVar, zzoh zzohVar) throws IOException {
        zznvVar.zzl();
        zznp zznpVar = (zznp) zznvVar;
        zznpVar.zzj(1, false);
        byte[] bArr = new byte[1];
        zznpVar.zzh(bArr, 0, 1, false);
        int i = bArr[0] & 1;
        zznpVar.zzj(2, false);
        int i2 = 1 != i ? 6 : 7;
        zzamf zzamfVar = new zzamf(i2);
        zzamfVar.zzf(zzny.zzb(zznvVar, zzamfVar.zzi(), 0, i2));
        zznvVar.zzl();
        zzob zzobVar = new zzob();
        if (zzd(zzamfVar, zzohVar, 1 == i, zzobVar)) {
            return zzobVar.zza;
        }
        throw zzaha.zzb(null, null);
    }

    public static int zzc(zzamf zzamfVar, int i) {
        switch (i) {
            case 1:
                return 192;
            case 2:
            case 3:
            case 4:
            case 5:
                return 576 << (i - 2);
            case 6:
                return zzamfVar.zzn() + 1;
            case 7:
                return zzamfVar.zzo() + 1;
            case 8:
            case 9:
            case 10:
            case 11:
            case 12:
            case 13:
            case 14:
            case 15:
                return 256 << (i - 8);
            default:
                return -1;
        }
    }

    private static boolean zzd(zzamf zzamfVar, zzoh zzohVar, boolean z, zzob zzobVar) {
        try {
            long jZzH = zzamfVar.zzH();
            if (!z) {
                jZzH *= (long) zzohVar.zzb;
            }
            zzobVar.zza = jZzH;
            return true;
        } catch (NumberFormatException unused) {
            return false;
        }
    }
}
