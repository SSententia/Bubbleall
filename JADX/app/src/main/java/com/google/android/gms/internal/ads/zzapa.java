...
public final class zzapa implements zzaob {
    ...
    public final void zze(ByteBuffer byteBuffer) {
        if (byteBuffer.hasRemaining()) {
            ...
            this.zzj += (long) iRemaining;
            this.zzd.zzc(shortBufferAsShortBuffer);
            byteBuffer.position(byteBuffer.position() + iRemaining);
        }
        int iZzf = this.zzd.zzf() * this.zzb;
        int i = iZzf + iZzf;
        if (i > 0) {
            if (this.zzg.capacity() < i) {
                ByteBuffer byteBufferOrder = ByteBuffer.allocateDirect(i).order(ByteOrder.nativeOrder());
                this.zzg = byteBufferOrder;
                this.zzh = byteBufferOrder.asShortBuffer();
            } else {
                this.zzg.clear();
                this.zzh.clear();
            }
    ...
    public final boolean zzh() {
        ...
        return zzaozVar == null || zzaozVar.zzf() == 0;
    }
    ...
    public final float zzk(float f) {
        float fZzg = zzave.zzg(f, 0.1f, 8.0f);
        this.zze = fZzg;
        return fZzg;
    }
