...
public final class zzdxq implements zzdbf, zzddx, zzdcu {
    ...
    zzdxq(zzdyc zzdycVar, zzfar zzfarVar) {
        this.zza = zzdycVar;
    ...
    private static JSONObject zze(zzdav zzdavVar) throws JSONException {
        ...
        if (((Boolean) zzbet.zzc().zzc(zzbjl.zzgG)).booleanValue()) {
            String strZzd = zzdavVar.zzd();
            if (!TextUtils.isEmpty(strZzd)) {
                String strValueOf = String.valueOf(strZzd);
                com.google.android.gms.ads.internal.util.zze.zzd(strValueOf.length() != 0 ? "Bidding data: ".concat(strValueOf) : new String("Bidding data: "));
                jSONObject.put("biddingData", new JSONObject(strZzd));
            }
        }
        JSONArray jSONArray = new JSONArray();
        List<zzbdp> listZzg = zzdavVar.zzg();
        if (listZzg != null) {
            for (zzbdp zzbdpVar : listZzg) {
    ...
    public final JSONObject zzd() throws JSONException {
        ...
        if (zzdavVar != null) {
            jSONObjectZze = zze(zzdavVar);
        } else {
            zzbcz zzbczVar = this.zzf;
            JSONObject jSONObjectZze2 = null;
            if (zzbczVar != null && (iBinder = zzbczVar.zze) != null) {
                zzdav zzdavVar2 = (zzdav) iBinder;
                jSONObjectZze2 = zze(zzdavVar2);
                List<zzbdp> listZzg = zzdavVar2.zzg();
                if (listZzg != null && listZzg.isEmpty()) {
                    JSONArray jSONArray = new JSONArray();
                    jSONArray.put(zzf(this.zzf));
                    jSONObjectZze2.put("errors", jSONArray);
                }
            }
            jSONObjectZze = jSONObjectZze2;
        }
        jSONObject.put("responseInfo", jSONObjectZze);
        return jSONObject;
    }
