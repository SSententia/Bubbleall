package com.google.android.gms.internal.ads;

import android.app.Activity;
import android.content.Context;
import android.net.NetworkCapabilities;
import android.os.Build;
import android.support.v4.media.session.PlaybackStateCompat;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.util.Log;
import android.view.MotionEvent;
import android.view.View;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

/* JADX INFO: compiled from: com.google.android.gms:play-services-ads@@20.5.0 */
/* JADX INFO: loaded from: classes2.dex */
public class zzaaq extends zzaap {
    protected static final Object zzr = new Object();
    static boolean zzs = false;
    private static final String zzw = "zzaaq";
    private static long zzx;
    private static zzaav zzy;
    protected final boolean zzt;
    protected final String zzu;
    zzaby zzv;

    protected zzaaq(Context context, String str, boolean z) {
        super(context);
        this.zzu = str;
        this.zzt = z;
    }

    protected static synchronized void zzf(Context context, boolean z) {
        if (zzs) {
            return;
        }
        zzx = System.currentTimeMillis() / 1000;
        zza = zzg(context, z);
        if (((Boolean) zzbet.zzc().zzc(zzbjl.zzbW)).booleanValue()) {
            zzy = zzaav.zzb(context);
        }
        zzs = true;
    }

    protected static zzabr zzg(Context context, boolean z) {
        if (zza == null) {
            synchronized (zzr) {
                if (zza == null) {
                    zzabr zzabrVarZzb = zzabr.zzb(context, "NYpdto3gBV8HiZtFXi3NN2dSfPyfe2T+8tUnAUjRH8A=", "", z);
                    if (zzabrVarZzb.zzd()) {
                        try {
                            if (((Boolean) zzbet.zzc().zzc(zzbjl.zzbP)).booleanValue()) {
                                zzabrVarZzb.zzp("wC9I8kYd+RKAHfQkBEAJYQw7avQUH+U9s6BQ/SJiEcKjDl2sT+FDEb6J3VYVY+ui", "FSkZmgTbP/gL1jHU7M+TcfgVpePk57vWMdHk6knGCa4=", new Class[0]);
                            }
                        } catch (IllegalStateException unused) {
                        }
                        zzabrVarZzb.zzp("NSYe0Ak7CUXd9zFZA3bczJ8pTgBK/kfUu9ICpHR+lQrTNc8+V7Owo49e2WIp0407", "Ux7t0A/7z2bV/IDvLZJgV4tTxr0Vvc1KngWKlG2Szwg=", Context.class);
                        zzabrVarZzb.zzp("Cm/m2hUfCdldkdMMT3yEm1sAGuFpKVfLQblDoR20XcK7ttTDkHIz3fwaKTv7az9S", "aEm/NgN//tNFhysboe/lIhu1ib9P4g8tTm8w79YCsSs=", Context.class);
                        zzabrVarZzb.zzp("6LxD9DBfXSlooSx7/LozL06rdHrjyh7Q8PNFejLJXxiRhvgA5vOTtEhIRp3pKXIi", "yXdkchwXd7KBwiPBiUydLTG3hsBC8U5EW7urXPkQrKc=", Context.class);
                        zzabrVarZzb.zzp("BEk7ZnZgUEtCcnEVmnqrsudDaU91B7wv3jOIcYFOdixfCZNd4CeUtXw9CNec1bEk", "yo4aeX8SQAYiyb+wCfW6Y7Ut6lHDx7k9wp/IaZ/McDY=", Context.class);
                        zzabrVarZzb.zzp("haIuUvChZNHM9MeHv9WrHlihODeJsTzWhRoU3l/WedulIO/DPlHeipCgsdOhWEK6", "2q9SJ42xLZKaCbpMEBENFcJEqDDES1vvcaask6iD3co=", Context.class);
                        zzabrVarZzb.zzp("0nCS1/lIiwmVLmY4a37W/6SChW3s76Q3PykiLxRk0eA7B+gs2O43l0EwWtwdYppM", "NOQ7v89FdlqbWITe5fQxaJU2MeOCrxMPjMVtpwyCEdc=", Context.class, Boolean.TYPE);
                        zzabrVarZzb.zzp("TDjQ/ClZ1A6U11LRle5Q8IpNZYzzk0FLHirJ3MQW6+Xmi/90o6gFxPcIONwzxlWm", "ZlhR2acRJ5DFhes+PG5pnG7AissVCA1YeE0si8KUOuk=", Context.class);
                        zzabrVarZzb.zzp("pECbn1i7ArZz8HseR71ZVkvFfTp84DNv2haWC1WmGQudMO74UcIxEa2NyELtzZhK", "LbaQh8VAFWEQeupBiMdSh8EgMse5yKT8p6jkCYTG+aQ=", Context.class);
                        zzabrVarZzb.zzp("6XqKWQ93+VsENz1XMuSK8tgooVS0F/+xtezkhSGK2kwZU9gOCstoEHJ4LdRwy+D0", "k9LiT4122PpqzUH6coaRvTZE6l9CWOGxr21WpJpkxjE=", MotionEvent.class, DisplayMetrics.class);
                        zzabrVarZzb.zzp("lJSjzU0WHYWPVV9sbEMt+ajOnNQwNtdM0PEq4BwIHEoSxWMr7EQ/IWk1GxHpKsqd", "fOkIRsWNRFrLLiTxAAtymkQwvThROsAenMGWOswUGPc=", MotionEvent.class, DisplayMetrics.class);
                        zzabrVarZzb.zzp("bwYfemyqKvs+5mX5RaQoUxmdyIG97sQWktW0fw649v7l/u+oM9fVxJ1I47AdFZo9", "ue4Q/YADEXoviiBHZurTF9IPPlfQKRVJdzRZ56oggWM=", new Class[0]);
                        zzabrVarZzb.zzp("kgLg33QaU762V6pmSw9NTYQDT3UN+SfZqKWL+LtHgczBAmljOlYdBxhmK655CLWm", "WLzuG2U+Kkbg0oKQlrLQGRQCiSGGQTHe1FZJKw3vRH0=", new Class[0]);
                        zzabrVarZzb.zzp("FIES3RTkQwHbrKX6yNHRLvjdTy/vAwaHt4NSjNSY8AdC8m3WKKtOY2UmKZKAKB0T", "2LDOSJy2Fx9VBMC+bm+0OJly9nmnJoeXDwyJmbeZyYc=", new Class[0]);
                        zzabrVarZzb.zzp("BZSpo1ki7Qq4VpNbKNY5xp3ObP5b46iKtLhMqj95i84Sl1pKZG1hW1hXSXh30EEu", "LLuZlwuRYspGUGo7OZU51ciMYYFQ89K2r1TLDzvNq9k=", new Class[0]);
                        zzabrVarZzb.zzp("A1tPgHG2ugzZJCs1M+dp+hmS1POsS+eh/W2v7YCpLDG/34A+E/oOE4ZeCFsAYJwW", "qStL+/rRmChGKiFwdmBpG5d//S+c8NqJKVwR4OiL/ms=", new Class[0]);
                        zzabrVarZzb.zzp("DVrAFpWxz2kcLonXDVqzAxxY5qZgb6+MyY8HJZUy9HF2czdhCSx5Lbv7dDVJMgy/", "AdToINYaWmfqYE6WezllyxILgCbRf0l/sf/vZralbBA=", new Class[0]);
                        zzabrVarZzb.zzp("NK1iwlHEHCICBCLEvTy0TnuhgEeSXovnPs9zKPvVW8trSfaaB+/inefY+5AxSSUI", "/qulcnd5BAOc2NixUFmrPgx+DAD1V/hpoK4nowHOBbg=", Context.class, Boolean.TYPE, String.class);
                        zzabrVarZzb.zzp("FW/4CvJE7gSaOIcNy9aPqAkZHcIOptuimuYgHtkFyrZcR+XexyybvWr2/z62L/0t", "gw9HX1g5D/FveKxEJmgn0Xdlo8h06mYt4u8/nHkUo04=", StackTraceElement[].class);
                        zzabrVarZzb.zzp("Rh7wPiyt0Q0zAefasWAQ/36LhULiVi3U1eCO9K34euxkll+3xBb3q2iaqyDuAy33", "Kmv9uscZLQkY9DcwgermeDphrIGnHnQJYdRAudw6Thg=", View.class, DisplayMetrics.class, Boolean.TYPE);
                        zzabrVarZzb.zzp("QXlICfiT7SVSRUb0DD1a74y++UqnSLMKlPOXAn9FNLrl7qN9uprYrk5dswjj/dLi", "yqGfvaGOT1fOScq8M0g9vywM6jvcTWdjxf27npfqtn0=", Context.class, Boolean.TYPE);
                        zzabrVarZzb.zzp("h8EVC95dwYnFQkP2MhkDIC/tXo0ux4wCcOlQmWH2MzSwaPfcZ/24kCQO32wp6PV0", "0H2C2EeQe84lGZgr+dAw2Fmmx+KWvzkBWNdP/wQOLp8=", View.class, Activity.class, Boolean.TYPE);
                        zzabrVarZzb.zzp("BF4UgSmqTS0MLe4qitQi/hj1jIQ+0vLIJEn32u0TtZn0VL9j15ZHt04Do4ADy833", "Yw5N4XiXXiTiwJwrJ8hW/AfPIwRQ2KMfiYKb9xu8vYQ=", Long.TYPE);
                        try {
                            if (((Boolean) zzbet.zzc().zzc(zzbjl.zzbU)).booleanValue()) {
                                zzabrVarZzb.zzp("Md0NasjzX5Dv6RV9gbRrdbbQw799E9EBpEgmAwiNqi/RiG7V51y0yTZI5hLTRiUF", "4FqMwpBmkecZ5KuntXganMUU8jtWDsP5C5fiOE4sCtY=", Context.class);
                            }
                        } catch (IllegalStateException unused2) {
                        }
                        zzabrVarZzb.zzp("Mx1UeFqV1fjeaaqnMs31Cpnz92KBTGXH/Fg2ftteXVEThFuI1yrtq0+LdsZyqokl", "Yj2yffKjPUt0Mx1uRMPIz4KPVLEfX/KYQGpvpNjX0eY=", Context.class);
                        try {
                            if (Build.VERSION.SDK_INT >= 26 && ((Boolean) zzbet.zzc().zzc(zzbjl.zzbW)).booleanValue()) {
                                zzabrVarZzb.zzp("E20AdmxNj0iK7Vx72nHXXjWnsmv4FUkyxC/Oztwx6eZIzlENIrQYdkN17JfC0Q1q", "rY9DNDEvlJE2YV76YVSJLP4cBUy/u1xcBXUcMxb1h8o=", NetworkCapabilities.class);
                            }
                        } catch (IllegalStateException unused3) {
                        }
                    }
                    zza = zzabrVarZzb;
                }
            }
        }
        return zza;
    }

    static zzabt zzh(zzabr zzabrVar, MotionEvent motionEvent, DisplayMetrics displayMetrics) throws zzabi {
        Method methodZzq = zzabrVar.zzq("6XqKWQ93+VsENz1XMuSK8tgooVS0F/+xtezkhSGK2kwZU9gOCstoEHJ4LdRwy+D0", "k9LiT4122PpqzUH6coaRvTZE6l9CWOGxr21WpJpkxjE=");
        if (methodZzq == null || motionEvent == null) {
            throw new zzabi();
        }
        try {
            return new zzabt((String) methodZzq.invoke(null, motionEvent, displayMetrics));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzabi(e);
        }
    }

    protected static final void zzr(List<Callable<Void>> list) {
        ExecutorService executorServiceZze;
        if (zza == null || (executorServiceZze = zza.zze()) == null || list.isEmpty()) {
            return;
        }
        try {
            executorServiceZze.invokeAll(list, ((Long) zzbet.zzc().zzc(zzbjl.zzbH)).longValue(), TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            Log.d(zzw, String.format("class methods got exception: %s", zzabu.zzd(e)));
        }
    }

    private final synchronized void zzs(zzabr zzabrVar, zzyj zzyjVar) {
        MotionEvent motionEvent;
        try {
            zzabt zzabtVarZzh = zzh(zzabrVar, this.zzb, this.zzq);
            Long l = zzabtVarZzh.zza;
            if (l != null) {
                zzyjVar.zzh(l.longValue());
            }
            Long l2 = zzabtVarZzh.zzb;
            if (l2 != null) {
                zzyjVar.zzi(l2.longValue());
            }
            Long l3 = zzabtVarZzh.zzc;
            if (l3 != null) {
                zzyjVar.zzj(l3.longValue());
            }
            if (this.zzp) {
                Long l4 = zzabtVarZzh.zzd;
                if (l4 != null) {
                    zzyjVar.zzv(l4.longValue());
                }
                Long l5 = zzabtVarZzh.zze;
                if (l5 != null) {
                    zzyjVar.zzw(l5.longValue());
                }
            }
        } catch (zzabi unused) {
        }
        zzyv zzyvVarZza = zzyw.zza();
        if (this.zzd > 0 && zzabu.zzg(this.zzq)) {
            zzyvVarZza.zzl(zzabu.zzh(this.zzk, 1, this.zzq));
            zzyvVarZza.zzm(zzabu.zzh(this.zzn - this.zzl, 1, this.zzq));
            zzyvVarZza.zzn(zzabu.zzh(this.zzo - this.zzm, 1, this.zzq));
            zzyvVarZza.zzq(zzabu.zzh(this.zzl, 1, this.zzq));
            zzyvVarZza.zzr(zzabu.zzh(this.zzm, 1, this.zzq));
            if (this.zzp && (motionEvent = this.zzb) != null) {
                long jZzh = zzabu.zzh(((this.zzl - this.zzn) + motionEvent.getRawX()) - this.zzb.getX(), 1, this.zzq);
                if (jZzh != 0) {
                    zzyvVarZza.zzo(jZzh);
                }
                long jZzh2 = zzabu.zzh(((this.zzm - this.zzo) + this.zzb.getRawY()) - this.zzb.getY(), 1, this.zzq);
                if (jZzh2 != 0) {
                    zzyvVarZza.zzp(jZzh2);
                }
            }
        }
        try {
            zzabt zzabtVarZzd = zzd(this.zzb);
            Long l6 = zzabtVarZzd.zza;
            if (l6 != null) {
                zzyvVarZza.zza(l6.longValue());
            }
            Long l7 = zzabtVarZzd.zzb;
            if (l7 != null) {
                zzyvVarZza.zzb(l7.longValue());
            }
            zzyvVarZza.zzh(zzabtVarZzd.zzc.longValue());
            if (this.zzp) {
                Long l8 = zzabtVarZzd.zze;
                if (l8 != null) {
                    zzyvVarZza.zzc(l8.longValue());
                }
                Long l9 = zzabtVarZzd.zzd;
                if (l9 != null) {
                    zzyvVarZza.zzf(l9.longValue());
                }
                Long l10 = zzabtVarZzd.zzf;
                if (l10 != null) {
                    zzyvVarZza.zzs(l10.longValue() != 0 ? 2 : 1);
                }
                if (this.zze > 0) {
                    Long lValueOf = zzabu.zzg(this.zzq) ? Long.valueOf(Math.round(this.zzj / this.zze)) : null;
                    if (lValueOf != null) {
                        zzyvVarZza.zzd(lValueOf.longValue());
                    } else {
                        zzyvVarZza.zze();
                    }
                    zzyvVarZza.zzg(Math.round(this.zzi / this.zze));
                }
                Long l11 = zzabtVarZzd.zzi;
                if (l11 != null) {
                    zzyvVarZza.zzj(l11.longValue());
                }
                Long l12 = zzabtVarZzd.zzj;
                if (l12 != null) {
                    zzyvVarZza.zzi(l12.longValue());
                }
                Long l13 = zzabtVarZzd.zzk;
                if (l13 != null) {
                    zzyvVarZza.zzt(l13.longValue() != 0 ? 2 : 1);
                }
            }
        } catch (zzabi unused2) {
        }
        long j = this.zzh;
        if (j > 0) {
            zzyvVarZza.zzk(j);
        }
        zzyjVar.zzI(zzyvVarZza.zzah());
        long j2 = this.zzd;
        if (j2 > 0) {
            zzyjVar.zzz(j2);
        }
        long j3 = this.zze;
        if (j3 > 0) {
            zzyjVar.zzy(j3);
        }
        long j4 = this.zzf;
        if (j4 > 0) {
            zzyjVar.zzx(j4);
        }
        long j5 = this.zzg;
        if (j5 > 0) {
            zzyjVar.zzA(j5);
        }
        try {
            int size = this.zzc.size() - 1;
            if (size > 0) {
                zzyjVar.zzK();
                for (int i = 0; i < size; i++) {
                    zzabt zzabtVarZzh2 = zzh(zza, this.zzc.get(i), this.zzq);
                    zzyv zzyvVarZza2 = zzyw.zza();
                    zzyvVarZza2.zza(zzabtVarZzh2.zza.longValue());
                    zzyvVarZza2.zzb(zzabtVarZzh2.zzb.longValue());
                    zzyjVar.zzJ(zzyvVarZza2.zzah());
                }
            }
        } catch (zzabi unused3) {
            zzyjVar.zzK();
        }
    }

    @Override // com.google.android.gms.internal.ads.zzaap
    protected final zzyj zza(Context context, zzyc zzycVar) {
        zzyj zzyjVarZzi = zzyz.zzi();
        if (!TextUtils.isEmpty(this.zzu)) {
            zzyjVarZzi.zzb(this.zzu);
        }
        zzabr zzabrVarZzg = zzg(context, this.zzt);
        if (zzabrVarZzg.zze() != null) {
            zzr(zzi(zzabrVarZzg, context, zzyjVarZzi, null));
        }
        return zzyjVarZzi;
    }

    @Override // com.google.android.gms.internal.ads.zzaap
    protected final zzyj zzb(Context context, View view, Activity activity) {
        zzyj zzyjVarZzi = zzyz.zzi();
        if (!TextUtils.isEmpty(this.zzu)) {
            zzyjVarZzi.zzb(this.zzu);
        }
        zzq(zzg(context, this.zzt), zzyjVarZzi, view, activity, true);
        return zzyjVarZzi;
    }

    @Override // com.google.android.gms.internal.ads.zzaap
    protected final zzyj zzc(Context context, View view, Activity activity) {
        zzyj zzyjVarZzi = zzyz.zzi();
        zzyjVarZzi.zzb(this.zzu);
        zzq(zzg(context, this.zzt), zzyjVarZzi, view, activity, false);
        return zzyjVarZzi;
    }

    @Override // com.google.android.gms.internal.ads.zzaap
    protected final zzabt zzd(MotionEvent motionEvent) throws zzabi {
        Method methodZzq = zza.zzq("lJSjzU0WHYWPVV9sbEMt+ajOnNQwNtdM0PEq4BwIHEoSxWMr7EQ/IWk1GxHpKsqd", "fOkIRsWNRFrLLiTxAAtymkQwvThROsAenMGWOswUGPc=");
        if (methodZzq == null || motionEvent == null) {
            throw new zzabi();
        }
        try {
            return new zzabt((String) methodZzq.invoke(null, motionEvent, this.zzq));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzabi(e);
        }
    }

    @Override // com.google.android.gms.internal.ads.zzaap
    protected final long zze(StackTraceElement[] stackTraceElementArr) throws zzabi {
        Method methodZzq = zza.zzq("FW/4CvJE7gSaOIcNy9aPqAkZHcIOptuimuYgHtkFyrZcR+XexyybvWr2/z62L/0t", "gw9HX1g5D/FveKxEJmgn0Xdlo8h06mYt4u8/nHkUo04=");
        if (methodZzq == null || stackTraceElementArr == null) {
            throw new zzabi();
        }
        try {
            return new zzabj((String) methodZzq.invoke(null, stackTraceElementArr)).zza.longValue();
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzabi(e);
        }
    }

    protected List<Callable<Void>> zzi(zzabr zzabrVar, Context context, zzyj zzyjVar, zzyc zzycVar) {
        int iZzu = zzabrVar.zzu();
        ArrayList arrayList = new ArrayList();
        if (!zzabrVar.zzd()) {
            zzyjVar.zzl(PlaybackStateCompat.ACTION_PREPARE);
            return arrayList;
        }
        arrayList.add(new zzacc(zzabrVar, "NK1iwlHEHCICBCLEvTy0TnuhgEeSXovnPs9zKPvVW8trSfaaB+/inefY+5AxSSUI", "/qulcnd5BAOc2NixUFmrPgx+DAD1V/hpoK4nowHOBbg=", zzyjVar, iZzu, 27, context, null));
        arrayList.add(new zzacf(zzabrVar, "bwYfemyqKvs+5mX5RaQoUxmdyIG97sQWktW0fw649v7l/u+oM9fVxJ1I47AdFZo9", "ue4Q/YADEXoviiBHZurTF9IPPlfQKRVJdzRZ56oggWM=", zzyjVar, zzx, iZzu, 25));
        arrayList.add(new zzacm(zzabrVar, "FIES3RTkQwHbrKX6yNHRLvjdTy/vAwaHt4NSjNSY8AdC8m3WKKtOY2UmKZKAKB0T", "2LDOSJy2Fx9VBMC+bm+0OJly9nmnJoeXDwyJmbeZyYc=", zzyjVar, iZzu, 1));
        arrayList.add(new zzacp(zzabrVar, "6LxD9DBfXSlooSx7/LozL06rdHrjyh7Q8PNFejLJXxiRhvgA5vOTtEhIRp3pKXIi", "yXdkchwXd7KBwiPBiUydLTG3hsBC8U5EW7urXPkQrKc=", zzyjVar, iZzu, 31));
        arrayList.add(new zzacu(zzabrVar, "kgLg33QaU762V6pmSw9NTYQDT3UN+SfZqKWL+LtHgczBAmljOlYdBxhmK655CLWm", "WLzuG2U+Kkbg0oKQlrLQGRQCiSGGQTHe1FZJKw3vRH0=", zzyjVar, iZzu, 33));
        arrayList.add(new zzacb(zzabrVar, "Cm/m2hUfCdldkdMMT3yEm1sAGuFpKVfLQblDoR20XcK7ttTDkHIz3fwaKTv7az9S", "aEm/NgN//tNFhysboe/lIhu1ib9P4g8tTm8w79YCsSs=", zzyjVar, iZzu, 29, context));
        arrayList.add(new zzacd(zzabrVar, "BEk7ZnZgUEtCcnEVmnqrsudDaU91B7wv3jOIcYFOdixfCZNd4CeUtXw9CNec1bEk", "yo4aeX8SQAYiyb+wCfW6Y7Ut6lHDx7k9wp/IaZ/McDY=", zzyjVar, iZzu, 5));
        arrayList.add(new zzacl(zzabrVar, "haIuUvChZNHM9MeHv9WrHlihODeJsTzWhRoU3l/WedulIO/DPlHeipCgsdOhWEK6", "2q9SJ42xLZKaCbpMEBENFcJEqDDES1vvcaask6iD3co=", zzyjVar, iZzu, 12));
        arrayList.add(new zzacn(zzabrVar, "0nCS1/lIiwmVLmY4a37W/6SChW3s76Q3PykiLxRk0eA7B+gs2O43l0EwWtwdYppM", "NOQ7v89FdlqbWITe5fQxaJU2MeOCrxMPjMVtpwyCEdc=", zzyjVar, iZzu, 3));
        arrayList.add(new zzace(zzabrVar, "BZSpo1ki7Qq4VpNbKNY5xp3ObP5b46iKtLhMqj95i84Sl1pKZG1hW1hXSXh30EEu", "LLuZlwuRYspGUGo7OZU51ciMYYFQ89K2r1TLDzvNq9k=", zzyjVar, iZzu, 44));
        arrayList.add(new zzaci(zzabrVar, "A1tPgHG2ugzZJCs1M+dp+hmS1POsS+eh/W2v7YCpLDG/34A+E/oOE4ZeCFsAYJwW", "qStL+/rRmChGKiFwdmBpG5d//S+c8NqJKVwR4OiL/ms=", zzyjVar, iZzu, 22));
        arrayList.add(new zzacv(zzabrVar, "TDjQ/ClZ1A6U11LRle5Q8IpNZYzzk0FLHirJ3MQW6+Xmi/90o6gFxPcIONwzxlWm", "ZlhR2acRJ5DFhes+PG5pnG7AissVCA1YeE0si8KUOuk=", zzyjVar, iZzu, 48));
        arrayList.add(new zzaca(zzabrVar, "pECbn1i7ArZz8HseR71ZVkvFfTp84DNv2haWC1WmGQudMO74UcIxEa2NyELtzZhK", "LbaQh8VAFWEQeupBiMdSh8EgMse5yKT8p6jkCYTG+aQ=", zzyjVar, iZzu, 49));
        arrayList.add(new zzacs(zzabrVar, "DVrAFpWxz2kcLonXDVqzAxxY5qZgb6+MyY8HJZUy9HF2czdhCSx5Lbv7dDVJMgy/", "AdToINYaWmfqYE6WezllyxILgCbRf0l/sf/vZralbBA=", zzyjVar, iZzu, 51));
        arrayList.add(new zzacq(zzabrVar, "QXlICfiT7SVSRUb0DD1a74y++UqnSLMKlPOXAn9FNLrl7qN9uprYrk5dswjj/dLi", "yqGfvaGOT1fOScq8M0g9vywM6jvcTWdjxf27npfqtn0=", zzyjVar, iZzu, 61));
        if (Build.VERSION.SDK_INT >= 24) {
            if (((Boolean) zzbet.zzc().zzc(zzbjl.zzbW)).booleanValue()) {
                arrayList.add(new zzack(zzabrVar, "E20AdmxNj0iK7Vx72nHXXjWnsmv4FUkyxC/Oztwx6eZIzlENIrQYdkN17JfC0Q1q", "rY9DNDEvlJE2YV76YVSJLP4cBUy/u1xcBXUcMxb1h8o=", zzyjVar, iZzu, 11, zzy));
            }
        }
        if (((Boolean) zzbet.zzc().zzc(zzbjl.zzbU)).booleanValue()) {
            arrayList.add(new zzaco(zzabrVar, "Md0NasjzX5Dv6RV9gbRrdbbQw799E9EBpEgmAwiNqi/RiG7V51y0yTZI5hLTRiUF", "4FqMwpBmkecZ5KuntXganMUU8jtWDsP5C5fiOE4sCtY=", zzyjVar, iZzu, 73));
        }
        arrayList.add(new zzacj(zzabrVar, "Mx1UeFqV1fjeaaqnMs31Cpnz92KBTGXH/Fg2ftteXVEThFuI1yrtq0+LdsZyqokl", "Yj2yffKjPUt0Mx1uRMPIz4KPVLEfX/KYQGpvpNjX0eY=", zzyjVar, iZzu, 76));
        return arrayList;
    }

    @Override // com.google.android.gms.internal.ads.zzaap, com.google.android.gms.internal.ads.zzaao
    public final void zzn(View view) {
        if (((Boolean) zzbet.zzc().zzc(zzbjl.zzbK)).booleanValue()) {
            if (this.zzv == null) {
                zzabr zzabrVar = zza;
                this.zzv = new zzaby(zzabrVar.zza, zzabrVar.zzm());
            }
            this.zzv.zza(view);
        }
    }

    /* JADX WARN: Code duplicated, block: B:7:0x002e  */
    protected final void zzq(zzabr zzabrVar, zzyj zzyjVar, View view, Activity activity, boolean z) {
        List listAsList;
        if (zzabrVar.zzd()) {
            zzs(zzabrVar, zzyjVar);
            ArrayList arrayList = new ArrayList();
            if (zzabrVar.zze() != null) {
                int iZzu = zzabrVar.zzu();
                arrayList.add(new zzach(zzabrVar, zzyjVar));
                arrayList.add(new zzacm(zzabrVar, "FIES3RTkQwHbrKX6yNHRLvjdTy/vAwaHt4NSjNSY8AdC8m3WKKtOY2UmKZKAKB0T", "2LDOSJy2Fx9VBMC+bm+0OJly9nmnJoeXDwyJmbeZyYc=", zzyjVar, iZzu, 1));
                arrayList.add(new zzacf(zzabrVar, "bwYfemyqKvs+5mX5RaQoUxmdyIG97sQWktW0fw649v7l/u+oM9fVxJ1I47AdFZo9", "ue4Q/YADEXoviiBHZurTF9IPPlfQKRVJdzRZ56oggWM=", zzyjVar, zzx, iZzu, 25));
                arrayList.add(new zzace(zzabrVar, "BZSpo1ki7Qq4VpNbKNY5xp3ObP5b46iKtLhMqj95i84Sl1pKZG1hW1hXSXh30EEu", "LLuZlwuRYspGUGo7OZU51ciMYYFQ89K2r1TLDzvNq9k=", zzyjVar, iZzu, 44));
                arrayList.add(new zzacl(zzabrVar, "haIuUvChZNHM9MeHv9WrHlihODeJsTzWhRoU3l/WedulIO/DPlHeipCgsdOhWEK6", "2q9SJ42xLZKaCbpMEBENFcJEqDDES1vvcaask6iD3co=", zzyjVar, iZzu, 12));
                arrayList.add(new zzacn(zzabrVar, "0nCS1/lIiwmVLmY4a37W/6SChW3s76Q3PykiLxRk0eA7B+gs2O43l0EwWtwdYppM", "NOQ7v89FdlqbWITe5fQxaJU2MeOCrxMPjMVtpwyCEdc=", zzyjVar, iZzu, 3));
                arrayList.add(new zzaci(zzabrVar, "A1tPgHG2ugzZJCs1M+dp+hmS1POsS+eh/W2v7YCpLDG/34A+E/oOE4ZeCFsAYJwW", "qStL+/rRmChGKiFwdmBpG5d//S+c8NqJKVwR4OiL/ms=", zzyjVar, iZzu, 22));
                arrayList.add(new zzacd(zzabrVar, "BEk7ZnZgUEtCcnEVmnqrsudDaU91B7wv3jOIcYFOdixfCZNd4CeUtXw9CNec1bEk", "yo4aeX8SQAYiyb+wCfW6Y7Ut6lHDx7k9wp/IaZ/McDY=", zzyjVar, iZzu, 5));
                arrayList.add(new zzacv(zzabrVar, "TDjQ/ClZ1A6U11LRle5Q8IpNZYzzk0FLHirJ3MQW6+Xmi/90o6gFxPcIONwzxlWm", "ZlhR2acRJ5DFhes+PG5pnG7AissVCA1YeE0si8KUOuk=", zzyjVar, iZzu, 48));
                arrayList.add(new zzaca(zzabrVar, "pECbn1i7ArZz8HseR71ZVkvFfTp84DNv2haWC1WmGQudMO74UcIxEa2NyELtzZhK", "LbaQh8VAFWEQeupBiMdSh8EgMse5yKT8p6jkCYTG+aQ=", zzyjVar, iZzu, 49));
                arrayList.add(new zzacs(zzabrVar, "DVrAFpWxz2kcLonXDVqzAxxY5qZgb6+MyY8HJZUy9HF2czdhCSx5Lbv7dDVJMgy/", "AdToINYaWmfqYE6WezllyxILgCbRf0l/sf/vZralbBA=", zzyjVar, iZzu, 51));
                arrayList.add(new zzacr(zzabrVar, "FW/4CvJE7gSaOIcNy9aPqAkZHcIOptuimuYgHtkFyrZcR+XexyybvWr2/z62L/0t", "gw9HX1g5D/FveKxEJmgn0Xdlo8h06mYt4u8/nHkUo04=", zzyjVar, iZzu, 45, new Throwable().getStackTrace()));
                arrayList.add(new zzacw(zzabrVar, "Rh7wPiyt0Q0zAefasWAQ/36LhULiVi3U1eCO9K34euxkll+3xBb3q2iaqyDuAy33", "Kmv9uscZLQkY9DcwgermeDphrIGnHnQJYdRAudw6Thg=", zzyjVar, iZzu, 57, view));
                arrayList.add(new zzacq(zzabrVar, "QXlICfiT7SVSRUb0DD1a74y++UqnSLMKlPOXAn9FNLrl7qN9uprYrk5dswjj/dLi", "yqGfvaGOT1fOScq8M0g9vywM6jvcTWdjxf27npfqtn0=", zzyjVar, iZzu, 61));
                if (((Boolean) zzbet.zzc().zzc(zzbjl.zzbI)).booleanValue()) {
                    arrayList.add(new zzabz(zzabrVar, "h8EVC95dwYnFQkP2MhkDIC/tXo0ux4wCcOlQmWH2MzSwaPfcZ/24kCQO32wp6PV0", "0H2C2EeQe84lGZgr+dAw2Fmmx+KWvzkBWNdP/wQOLp8=", zzyjVar, iZzu, 62, view, activity));
                }
                if (z && ((Boolean) zzbet.zzc().zzc(zzbjl.zzbK)).booleanValue()) {
                    arrayList.add(new zzact(zzabrVar, "BF4UgSmqTS0MLe4qitQi/hj1jIQ+0vLIJEn32u0TtZn0VL9j15ZHt04Do4ADy833", "Yw5N4XiXXiTiwJwrJ8hW/AfPIwRQ2KMfiYKb9xu8vYQ=", zzyjVar, iZzu, 53, this.zzv));
                }
            }
            listAsList = arrayList;
        } else {
            zzyjVar.zzl(PlaybackStateCompat.ACTION_PREPARE);
            listAsList = Arrays.asList(new zzach(zzabrVar, zzyjVar));
        }
        zzr(listAsList);
    }
}
