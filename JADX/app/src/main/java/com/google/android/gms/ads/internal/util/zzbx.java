                ...
                if (objOpt instanceof Boolean) {
                    ...
                    if (jSONArray.length() != 0) {
                        ...
                        if (objOpt2 == null) {
                            ...
                            zze.zzi(strValueOf.length() != 0 ? "Expected JSONArray with at least 1 non-null element for key:".concat(strValueOf) : new String("Expected JSONArray with at least 1 non-null element for key:"));
                        } else if (objOpt2 instanceof JSONObject) {
                            Bundle[] bundleArr = new Bundle[length];
                            while (i < length) {
                                bundleArr[i] = !jSONArray.isNull(i) ? zzk(jSONArray.optJSONObject(i)) : null;
                                i++;
                            }
                            ...
                            zze.zzi(String.format("JSONArray with unsupported type %s for key:%s", objOpt2.getClass().getCanonicalName(), next));
                        }
                    }
                } else if (objOpt instanceof JSONObject) {
                    bundle.putBundle(next, zzk((JSONObject) objOpt));
                } else {
                    String strValueOf2 = String.valueOf(next);
                    zze.zzi(strValueOf2.length() != 0 ? "Unsupported type for key:".concat(strValueOf2) : new String("Unsupported type for key:"));
                }
            }
        }
        return bundle;
