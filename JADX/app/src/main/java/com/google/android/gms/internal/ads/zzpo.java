package com.google.android.gms.internal.ads;

import java.util.ArrayList;
import java.util.Date;
import java.util.HashMap;
import java.util.List;
import java.util.Map;

/* JADX INFO: compiled from: com.google.android.gms:play-services-ads@@20.5.0 */
/* JADX INFO: loaded from: classes2.dex */
final class zzpo extends zzpq {
    private long zzb;
    private long[] zzc;
    private long[] zzd;

    public zzpo() {
        super(new zznt());
        this.zzb = -9223372036854775807L;
        this.zzc = new long[0];
        this.zzd = new long[0];
    }

    private static Double zzg(zzamf zzamfVar) {
        return Double.valueOf(Double.longBitsToDouble(zzamfVar.zzx()));
    }

    private static String zzh(zzamf zzamfVar) {
        int iZzo = zzamfVar.zzo();
        int iZzg = zzamfVar.zzg();
        zzamfVar.zzk(iZzo);
        return new String(zzamfVar.zzi(), iZzg, iZzo);
    }

    private static HashMap<String, Object> zzi(zzamf zzamfVar) {
        int iZzB = zzamfVar.zzB();
        HashMap<String, Object> map = new HashMap<>(iZzB);
        for (int i = 0; i < iZzB; i++) {
            String strZzh = zzh(zzamfVar);
            Object objZzj = zzj(zzamfVar, zzamfVar.zzn());
            if (objZzj != null) {
                map.put(strZzh, objZzj);
            }
        }
        return map;
    }

    @Override // com.google.android.gms.internal.ads.zzpq
    protected final boolean zza(zzamf zzamfVar) {
        return true;
    }

    @Override // com.google.android.gms.internal.ads.zzpq
    protected final boolean zzb(zzamf zzamfVar, long j) {
        if (zzamfVar.zzn() != 2 || !"onMetaData".equals(zzh(zzamfVar)) || zzamfVar.zzn() != 8) {
            return false;
        }
        HashMap<String, Object> mapZzi = zzi(zzamfVar);
        Object obj = mapZzi.get("duration");
        if (obj instanceof Double) {
            double dDoubleValue = ((Double) obj).doubleValue();
            if (dDoubleValue > 0.0d) {
                this.zzb = (long) (dDoubleValue * 1000000.0d);
            }
        }
        Object obj2 = mapZzi.get("keyframes");
        if (obj2 instanceof Map) {
            Map map = (Map) obj2;
            Object obj3 = map.get("filepositions");
            Object obj4 = map.get("times");
            if ((obj3 instanceof List) && (obj4 instanceof List)) {
                List list = (List) obj3;
                List list2 = (List) obj4;
                int size = list2.size();
                this.zzc = new long[size];
                this.zzd = new long[size];
                for (int i = 0; i < size; i++) {
                    Object obj5 = list.get(i);
                    Object obj6 = list2.get(i);
                    if (!(obj6 instanceof Double) || !(obj5 instanceof Double)) {
                        this.zzc = new long[0];
                        this.zzd = new long[0];
                        break;
                    }
                    this.zzc[i] = (long) (((Double) obj6).doubleValue() * 1000000.0d);
                    this.zzd[i] = ((Double) obj5).longValue();
                }
            }
        }
        return false;
    }

    public final long zzc() {
        return this.zzb;
    }

    public final long[] zzd() {
        return this.zzc;
    }

    public final long[] zze() {
        return this.zzd;
    }

    private static Object zzj(zzamf zzamfVar, int i) {
        if (i == 0) {
            return zzg(zzamfVar);
        }
        if (i == 1) {
            return Boolean.valueOf(zzamfVar.zzn() == 1);
        }
        if (i == 2) {
            return zzh(zzamfVar);
        }
        if (i != 3) {
            if (i == 8) {
                return zzi(zzamfVar);
            }
            if (i != 10) {
                if (i != 11) {
                    return null;
                }
                Date date = new Date((long) zzg(zzamfVar).doubleValue());
                zzamfVar.zzk(2);
                return date;
            }
            int iZzB = zzamfVar.zzB();
            ArrayList arrayList = new ArrayList(iZzB);
            for (int i2 = 0; i2 < iZzB; i2++) {
                Object objZzj = zzj(zzamfVar, zzamfVar.zzn());
                if (objZzj != null) {
                    arrayList.add(objZzj);
                }
            }
            return arrayList;
        }
        HashMap map = new HashMap();
        while (true) {
            String strZzh = zzh(zzamfVar);
            int iZzn = zzamfVar.zzn();
            if (iZzn == 9) {
                return map;
            }
            Object objZzj2 = zzj(zzamfVar, iZzn);
            if (objZzj2 != null) {
                map.put(strZzh, objZzj2);
            }
        }
    }
}
