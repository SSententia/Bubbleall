    ...
    public void load(Map<String, String> map) throws MappingException {
        ...
        for (Field field3 : map2.values()) {
            if (((Parameter) field3.getAnnotation(Parameter.class)).required()) {
                ...
                zzcgt.zzi(strValueOf.length() != 0 ? "Required server option missing: ".concat(strValueOf) : new String("Required server option missing: "));
                if (sb4.length() > 0) {
                    sb4.append(", ");
                }
        ...
        if (sb4.length() > 0) {
            ...
            throw new MappingException(string.length() != 0 ? "Required server option(s) missing: ".concat(string) : new String("Required server option(s) missing: "));
        }
    }
}
