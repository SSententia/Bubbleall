...
class SystemJobInfoConverter {
    ...
    JobInfo convert(WorkSpec workSpec, int jobId) {
        ...
        persistableBundle.putString(EXTRA_WORK_SPEC_ID, workSpec.f102id);
    ...
    private static JobInfo.TriggerContentUri convertContentUriTrigger(ContentUriTriggers.Trigger trigger) {
        ...
        Trace$$ExternalSyntheticApiModelOutline0.m302m();
        return Trace$$ExternalSyntheticApiModelOutline0.m277m(trigger.getUri(), zShouldTriggerForDescendants ? 1 : 0);
    }
    ...
    static int convertNetworkType(NetworkType networkType) {
        int i = C10441.$SwitchMap$androidx$work$NetworkType[networkType.ordinal()];
        if (i == 1) {
            return 0;
        }
        if (i == 2) {
