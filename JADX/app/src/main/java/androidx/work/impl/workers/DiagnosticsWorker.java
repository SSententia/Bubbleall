...
public class DiagnosticsWorker extends Worker {
    ...
    private static String workSpecRows(WorkNameDao workNameDao, WorkTagDao workTagDao, SystemIdInfoDao systemIdInfoDao, List<WorkSpec> workSpecs) {
        ...
        for (WorkSpec workSpec : workSpecs) {
            SystemIdInfo systemIdInfo = systemIdInfoDao.getSystemIdInfo(workSpec.f102id);
            sb.append(workSpecRow(workSpec, TextUtils.join(",", workNameDao.getNamesForWorkSpecId(workSpec.f102id)), systemIdInfo != null ? Integer.valueOf(systemIdInfo.systemId) : null, TextUtils.join(",", workTagDao.getTagsForWorkSpecId(workSpec.f102id))));
        }
        return sb.toString();
    }

    private static String workSpecRow(WorkSpec workSpec, String name, Integer systemId, String tags) {
        return String.format("\n%s\t %s\t %s\t %s\t %s\t %s\t", workSpec.f102id, workSpec.workerClassName, systemId, workSpec.state.name(), name, tags);
    }
}
