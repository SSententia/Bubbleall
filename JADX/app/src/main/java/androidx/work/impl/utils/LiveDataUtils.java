...
public class LiveDataUtils {
    public static <In, Out> LiveData<Out> dedupedMappedLiveDataFor(LiveData<In> inputLiveData, final Function<In, Out> mappingMethod, final TaskExecutor workTaskExecutor) {
        ...
        mediatorLiveData.addSource(inputLiveData, new C10821(workTaskExecutor, obj, mappingMethod, mediatorLiveData));
        return mediatorLiveData;
    }

    /* JADX INFO: Add missing generic type declarations: [In] */
    /* JADX INFO: renamed from: androidx.work.impl.utils.LiveDataUtils$1 */
    class C10821<In> implements Observer<In> {
        ...
        C10821(final TaskExecutor val$workTaskExecutor, final Object val$lock, final Function val$mappingMethod, final MediatorLiveData val$outputLiveData) {
        ...
        public void onChanged(final In input) {
            this.val$workTaskExecutor.executeOnBackgroundThread(new Runnable() { // from class: androidx.work.impl.utils.LiveDataUtils.1.1
                ...
                public void run() {
                    synchronized (C10821.this.val$lock) {
                        ?? Apply = C10821.this.val$mappingMethod.apply(input);
                        if (C10821.this.mCurrentOutput == 0 && Apply != 0) {
                            C10821.this.mCurrentOutput = Apply;
                            C10821.this.val$outputLiveData.postValue(Apply);
                        } else if (C10821.this.mCurrentOutput != 0 && !C10821.this.mCurrentOutput.equals(Apply)) {
                            C10821.this.mCurrentOutput = Apply;
                            C10821.this.val$outputLiveData.postValue(Apply);
                        }
                    }
