            ...
            if (workSpec.state == WorkInfo.State.ENQUEUED) {
                if (jCurrentTimeMillis < jCalculateNextRunTime) {
                    ...
                    if (workSpec.constraints.requiresDeviceIdle()) {
                        ...
                        hashSet2.add(workSpec.f102id);
                    }
                } else {
                    Logger.get().debug(TAG, String.format("Starting work for %s", workSpec.f102id), new Throwable[0]);
                    this.mWorkManagerImpl.startWork(workSpec.f102id);
                }
            }
        }
    ...
    private void removeConstraintTrackingFor(String workSpecId) {
        synchronized (this.mLock) {
            for (WorkSpec workSpec : this.mConstrainedWorkSpecs) {
                if (workSpec.f102id.equals(workSpecId)) {
