            ...
            if (Build.VERSION.SDK_INT >= 30) {
                ...
                if (historicalProcessExitReasons != null && !historicalProcessExitReasons.isEmpty()) {
                    for (int i = 0; i < historicalProcessExitReasons.size(); i++) {
                        if (zzn$$ExternalSyntheticApiModelOutline0.m350m(historicalProcessExitReasons.get(i)).getReason() == 10) {
                            return true;
                        }
                    }
                }
        ...
        try {
            ...
            if (z) {
                for (WorkSpec workSpec : runningWork) {
                    workSpecDao.setState(WorkInfo.State.ENQUEUED, workSpec.f102id);
                    workSpecDao.markWorkSpecScheduled(workSpec.f102id, -1L);
                }
            }
