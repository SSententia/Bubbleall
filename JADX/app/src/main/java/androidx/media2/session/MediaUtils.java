...
public class MediaUtils {
    ...
    public static MediaItem convertToMediaItem(MediaMetadataCompat mediaMetadataCompat, int i) {
        ...
        MediaMetadata.Builder builderPutRating = new MediaMetadata.Builder().putLong(MediaMetadata.METADATA_KEY_PLAYABLE, 1L).putRating("android.media.metadata.USER_RATING", convertToRating(RatingCompat.newUnratedRating(i)));
        for (String str : mediaMetadataCompat.keySet()) {
            ...
            if (obj instanceof CharSequence) {
                builderPutRating.putText(str2, (CharSequence) obj);
            } else if (obj instanceof Bitmap) {
                builderPutRating.putBitmap(str2, (Bitmap) obj);
            } else if (obj instanceof Long) {
                builderPutRating.putLong(str2, ((Long) obj).longValue());
            } else if ((obj instanceof RatingCompat) || (obj instanceof Rating)) {
                try {
                    builderPutRating.putRating(str2, convertToRating(mediaMetadataCompat.getRating(str)));
                } catch (Exception unused) {
                }
            }
        }
        return new MediaItem.Builder().setMetadata(builderPutRating.build()).build();
    }
    ...
    public static List<MediaItem> convertQueueItemListToMediaItemList(List<MediaSessionCompat.QueueItem> list) {
        ...
        ArrayList arrayList = new ArrayList();
        for (int i = 0; i < list.size(); i++) {
            MediaItem mediaItemConvertToMediaItem = convertToMediaItem(list.get(i));
    ...
    public static List<MediaSessionCompat.QueueItem> convertToQueueItemList(List<MediaItem> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            arrayList.add(new MediaSessionCompat.QueueItem(description, convertToQueueItemId(i)));
        }
        return arrayList;
    }
    ...
    static boolean doesBundleHaveCustomParcelable(Bundle bundle) {
        Parcel parcelObtain = Parcel.obtain();
        try {
