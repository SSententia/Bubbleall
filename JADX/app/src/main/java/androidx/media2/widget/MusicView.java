...
class MusicView extends ViewGroup {
    ...
    protected void onMeasure(int i, int i2) {
        ...
        if (this.mType == MusicViewType.WITHOUT_TITLE) {
            this.mWithoutTitle.measure(View.MeasureSpec.makeMeasureSpec(size / 2, BasicMeasure.EXACTLY), View.MeasureSpec.makeMeasureSpec(size2 / 2, BasicMeasure.EXACTLY));
        }
    ...
    void setAlbumDrawable(Drawable drawable) {
        int childCount = getChildCount();
        for (int i = 0; i < childCount; i++) {
            ImageView imageView = (ImageView) getChildAt(i).findViewById(C0793R.id.album);
            if (imageView != null) {
    ...
    void setTitleText(String str) {
        int childCount = getChildCount();
        for (int i = 0; i < childCount; i++) {
            TextView textView = (TextView) getChildAt(i).findViewById(C0793R.id.title);
            if (textView != null) {
    ...
    void setArtistText(String str) {
        int childCount = getChildCount();
        for (int i = 0; i < childCount; i++) {
            TextView textView = (TextView) getChildAt(i).findViewById(C0793R.id.artist);
            if (textView != null) {
                textView.setText(str);
            }
        }
    ...
    private void inflateLayout() {
        ...
        this.mWithTitleLandscape = layoutInflater.inflate(C0793R.layout.media2_widget_music_with_title_landscape, (ViewGroup) null);
        this.mWithTitlePortrait = layoutInflater.inflate(C0793R.layout.media2_widget_music_with_title_portrait, (ViewGroup) null);
        this.mWithoutTitle = layoutInflater.inflate(C0793R.layout.media2_widget_music_without_title, (ViewGroup) null);
