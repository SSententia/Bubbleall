...
class PathInterpolatorApi14 implements Interpolator {
    ...
    private final float[] f67mX;

    /* JADX INFO: renamed from: mY */
    private final float[] f68mY;
    ...
    PathInterpolatorApi14(Path path) {
        ...
        int i2 = i + 1;
        this.f67mX = new float[i2];
        this.f68mY = new float[i2];
        float[] fArr = new float[2];
        for (int i3 = 0; i3 < i2; i3++) {
            pathMeasure.getPosTan((i3 * length) / i, fArr, null);
            this.f67mX[i3] = fArr[0];
            this.f68mY[i3] = fArr[1];
        }
    }
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f67mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f67mX[i2]) {
